...
function ContasReceber() {
  ...
  const [pedidos, setPedidos] = useState([]); // Para o formulário do modal
  ...
  // Efeito para buscar os dados da API de forma segura
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      setError(null);
      try {
        // 1. Carrega os dados ESSENCIAIS para a tabela. Se isto falhar, a página mostra um erro.
        const contasResponse = await api.get('/contas-receber');
        setContas(contasResponse.data || []);

        // 2. Tenta carregar os dados SECUNDÁRIOS para o formulário. Se falhar, a página ainda funciona.
        try {
          const pedidosResponse = await api.get('/pedidos');
          setPedidos(pedidosResponse.data || []);
        } catch (formError) {
          console.warn("Aviso: Não foi possível carregar a lista de pedidos para o formulário.", formError.message);
          // Não definimos um erro principal aqui para não quebrar a página.
        }

      } catch (mainError) {
        setError("Falha ao carregar contas a receber. Verifique se a API está online e a rota '/contas-receber' existe.");
        ...
        <form onSubmit={handleFormSubmit}>
          <div style={formGroupStyle}>
            ...
            <select 
              ...
              style={inputStyle} 
              required
              disabled={!pedidos.length} // Desabilita se a lista de pedidos não carregou
            >
              <option value="">
